...
function openFile() {
    ...
    var csvData = fs.readFileSync('C:/Users/DamiánTrinca/Downloads/Analisis DB(SpDefinitionDbClientes).csv', 'utf8').toString();
    ...
    for (var _i = 0, lines_1 = lines; _i < lines_1.length; _i++) {
        ...
        var name_1 = values.shift();
        dataArray.push({ name: name_1, sp: values.join(';') });
    }
    console.log({ dataArray: dataArray });
    // const regex = /sp(?!\s)/; // Matches 'sp' not followed by a space
    var data = dataArray.filter(function (data) { return data.sp.toLowerCase().includes('exec sp') || data.sp.toLowerCase().includes('exec  sp'); });
    var filterSp = data.map(function (p) {
        // const inputString = p.sp;
        // const regex = /exec\s(\S+)/; // Matches 'exec ' followed by non-space characters
        // const match = inputString.match(regex);
        var result = p.sp.toLowerCase().split('exec ').filter(function (p) { return p.toLowerCase().includes('sp'); });
    ...
    var final = [];
    filterSp.forEach(function (p) {
        var newResult = [];
        p.result.forEach(function (q) {
    ...
    final.forEach(function (p) {
        p.sps.forEach(function (q) {
            stringFinal.push("".concat(q, ",").concat(p.name));
            myLogger.log("".concat(q, ",").concat(p.name));
        });
    });
    var stringSet = new Set(stringFinal);
